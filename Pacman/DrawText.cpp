...
void DrawText(bool i_center, unsigned short i_x, unsigned short i_y, const std::string& i_text, sf::RenderWindow& i_window)
{
	...
	if (1 == i_center)
	{
		// I'm not even going to pretend to know what this does. But it works so i'm not touching it.
	...
	for (std::string::const_iterator a = i_text.begin(); a != i_text.end(); a++)
	{
		if ('\n' == *a)
		{
			if (1 == i_center)
			{
				// Don't question it. I don't understand it either, if you do please explain.
		...
		// The font texture begins with a space, which is the 32nd character.
